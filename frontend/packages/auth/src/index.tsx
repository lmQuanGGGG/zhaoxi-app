...
export async function refreshServerSession(){const current=readSession();const controller=new AbortController();const timeout=window.setTimeout(()=>controller.abort(),12000);try{const response=await fetch("/api/auth/unified/session/me",{cache:"no-store",signal:controller.signal});const payload=await response.json().catch(()=>null);if(response.ok&&payload?.ok){saveServerSession(payload.data,current?.role);return readSession();}/* A transient backend/network failure must never erase a valid local session. */if(response.status!==401&&response.status!==403)return current;return null;}catch{return current;}finally{window.clearTimeout(timeout);}}
