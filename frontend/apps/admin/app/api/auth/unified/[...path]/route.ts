...
async function handle(request:NextRequest,context:{params:Promise<{path:string[]}>}){
 const {path:parts}=await context.params;const path=(parts||[]).join("/");if(!(path.startsWith("session/")||path.startsWith("qr/")||path==="guest/bootstrap"||path==="admin/card"))return NextResponse.json({ok:false,error:{message:"Unsupported auth route",code:"AUTH_ROUTE_NOT_FOUND"}},{status:404});
 ...
 let r=await upstream(path,method,body,access); let payload=await r.json();
 if(r.status===401 && method==="GET" && (path==="session/me"||path==="session/devices")){const refreshed=await refreshWithCookie(request);if(refreshed){access=String(refreshed.accessToken||"");r=await upstream(path,method,undefined,access);payload=await r.json();const response=NextResponse.json(payload,{status:r.status,headers:{"cache-control":"no-store"}});setAuthCookies(response,refreshed);return response;}}
 if(path==="session/exchange"||path==="qr/exchange"||path==="guest/bootstrap"||path==="admin/card"||path==="session/refresh"){const response=NextResponse.json(payload?.ok?{ok:true,data:payload.data.session}:payload,{status:r.status,headers:{"cache-control":"no-store"}});if(payload?.ok)setAuthCookies(response,payload.data);if(payload.data?.trustedDeviceToken)response.cookies.set(TRUSTED_COOKIE,String(payload.data.trustedDeviceToken),cookieOptions(180*24*60*60));return response;}
