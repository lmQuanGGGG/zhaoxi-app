...
const copy = {
  "zh-CN": {
    ...
    preview: "预览", saved: "已保存", uploadFailed: "图片上传失败", details: "服务详情", deleteItem: "删除项目", deleteConfirm: "确定删除这个项目吗？", logout: "退出登录", confirmBanners: "确认轮播图", bannersConfirmed: "轮播图已确认", syncServices: "同步服务", syncing: "同步中…", synced: "已同步到客户端", syncHint: "确认轮播图和商品后再同步到客户端。", lockItem: "锁定菜品", unlockItem: "重新上架", available: "供应中", soldOut: "已售罄", bannerPreview: "轮播图预览", imageUploaded: "图片已上传", address: "地址", contactPhone: "联系电话", wechat: "微信", infoTab: "商家资料", ordersTab: "订单管理", housingGallery:"房源相册", addGallery:"添加房源图片", galleryHint:"可上传多张图片用于客户左右滑动查看。", loadingStore:"正在加载店铺", loadingStoreHint:"完成后内容会立即更新。", chooseOrPasteImage:"选择图片或粘贴网址", imageWillUpload:"图片会在保存列表时上传", imageRemoved:"已从此商品移除图片。", imageUrlWillUse:"保存列表时将使用此图片网址。", itemReadyToSave:"已加入列表。图片尚未上传；继续添加商品，完成后保存列表。", requiredItemFields:"请填写：",
  ...
  "zh-TW": {
    ...
    preview: "預覽", saved: "已儲存", uploadFailed: "圖片上傳失敗", details: "服務詳情", deleteItem: "刪除項目", deleteConfirm: "確定刪除這個項目嗎？", logout: "登出", confirmBanners: "確認輪播圖", bannersConfirmed: "輪播圖已確認", syncServices: "同步服務", syncing: "同步中…", synced: "已同步到客戶端", syncHint: "確認輪播圖與項目後再同步到客戶端。", lockItem: "鎖定餐點", unlockItem: "重新上架", available: "供應中", soldOut: "已售罄", bannerPreview: "輪播圖預覽", imageUploaded: "圖片已上傳", address: "地址", contactPhone: "聯絡電話", wechat: "微信", infoTab: "商家資料", ordersTab: "訂單管理", housingGallery:"房源相簿", addGallery:"新增房源圖片", galleryHint:"可上傳多張圖片供客戶左右滑動查看。", loadingStore:"正在載入店鋪", loadingStoreHint:"完成後內容會立即更新。", chooseOrPasteImage:"選擇圖片或貼上網址", imageWillUpload:"圖片會在儲存列表時上傳", imageRemoved:"已從此商品移除圖片。", imageUrlWillUse:"儲存列表時將使用此圖片網址。", itemReadyToSave:"已加入列表。圖片尚未上傳；繼續新增商品，完成後儲存列表。", requiredItemFields:"請填寫：",
...
export default function StoreManager() {
  ...
  async function chooseItemImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    setItemNotice(`✓ ${t.imageWillUpload}`);
    event.target.value = "";
  }
  ...
  function queueItem() {
    ...
    if (invalid) {
      ...
      setMsg(message);
      setItemNotice(form.name.trim() && form.price.trim() && !form.image.trim() ? t.chooseOrPasteImage : message);
      return;
    }
    ...
    setItemPreview("");
    setMsg(`✓ ${queueText}`);
    setItemNotice(`✓ ${t.itemReadyToSave}`);
  }
  ...
  return <main className="zx-store-manager" style={{ ...appShellStyle, maxWidth: 1100, margin: "0 auto", padding: 20 }}>
    ...
    {isLoadingStore && <div className="zx-store-loading" role="status" aria-live="polite"><span className="zx-store-loading-spinner" /><div><b>{t.loadingStore}</b><small>{t.loadingStoreHint}</small></div></div>}
    ...
    <Surface style={{ marginTop: 18 }}>
      ...
      <section style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><ActionButton tone="neutral" onClick={() => logoInput.current?.click()}>{uploading === "logo" ? t.uploading : t.chooseImage}</ActionButton>{logo && <ActionButton tone="neutral" onClick={() => { if(window.confirm(t.deleteConfirm)){setLogo("");setLogoPreview("");} }}>{t.remove}</ActionButton>}</div>
      ...
      <section style={{ display: "grid", gap: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 10 }}>{visibleBanners.map((url, index) => <div key={`${url}-${index}`}><ImagePreview url={url} alt={`${presentation.banners} ${index + 1}`} height={100} /><button type="button" onClick={() => { if(window.confirm(t.deleteConfirm)){setBannerUrls((items) => items.filter((_, i) => i !== index));setBannersConfirmed(false);} }} style={{ marginTop: 5 }}>{t.remove}</button></div>)}</div>
    ...
    <Surface style={{ marginTop: 18 }}>
      ...
      <p style={{ margin: "0 0 12px", color: "#64748b", fontSize: 14 }}><b style={{ color: "#dc2626" }}>*</b> {requiredText}: {t.name}, {t.price}, {presentation.image} ({t.chooseOrPasteImage}). {t.summary} ({optionalText}).</p>
      ...
      <section style={{ display: "grid", gap: 8, marginTop: 14 }}><b>{presentation.image} <span style={{ color: "#dc2626" }}>* {requiredText}</span> <small>({t.chooseOrPasteImage})</small></b><ImagePreview url={visibleItemImage} alt={form.name || presentation.image} height={180} /><input ref={itemInput} type="file" accept="image/jpeg,image/png,image/webp,image/gif" hidden onChange={chooseItemImage} /><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><ActionButton tone="neutral" onClick={() => itemInput.current?.click()}>{t.chooseImage}</ActionButton>{(form.image || form.imageFile) && <ActionButton tone="neutral" onClick={() => { if (form.imagePreview) URL.revokeObjectURL(form.imagePreview); setForm({ ...form, image: "", imageFile: undefined, imagePreview: undefined }); setItemPreview(""); setItemNotice(t.imageRemoved); }}>{t.remove}</ActionButton>}</div><input aria-label={`${t.urlFallback} (${optionalText})`} placeholder={`${t.urlFallback} (${optionalText})`} value={form.image} onChange={(e) => { if (form.imagePreview) URL.revokeObjectURL(form.imagePreview); setItemPreview(""); setForm({ ...form, image: e.target.value, imageFile: undefined, imagePreview: undefined }); setItemNotice(e.target.value.trim() ? `✓ ${t.imageUrlWillUse}` : ""); }} /></section>
      ...
      {queuedItems.length > 0 && <div style={{ marginTop: 12, display: "grid", gap: 7 }}>{queuedItems.map((item, index) => <div key={`${item.name}-${index}`} style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", padding: "9px 11px", border: "1px solid #dbe4df", borderRadius: 10 }}><div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}><img src={item.image || item.imagePreview} alt={item.name} style={{ width: 54, height: 54, flex: "0 0 auto", objectFit: "cover", borderRadius: 9, border: "1px solid #dbe4df" }} /><span style={{ minWidth: 0 }}><b>{index + 1}. {item.name}</b><small style={{ display: "block", color: "#64748b", marginTop: 3 }}>{t.imageWillUpload}</small><span style={{ display: "block", marginTop: 3 }}>{Number(item.price).toLocaleString(locale)} VND</span></span></div><button type="button" onClick={() => { if (item.imagePreview) URL.revokeObjectURL(item.imagePreview); setQueuedItems((current) => current.filter((_, i) => i !== index)); }}>{t.remove}</button></div>)}</div>}
    </Surface>
