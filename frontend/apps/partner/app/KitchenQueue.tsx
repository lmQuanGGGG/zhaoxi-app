...
type Item={requestId:string;requestCode:string;stage:string;priority:string;serviceName:string;customerName:string;customerPhone?:string;addressText?:string;quantity:number;estimatedMinutes:number;estimatedReadyAt:string|null;overdueMinutes:number;elapsedMinutes:number;late:boolean;courierName:string;deliveryProvider?:string;deliveryProviderLabel?:string;sortScore:number};
type Data={counts:{waiting:number;preparing:number;ready:number;courier:number;late:number};items:Item[]};
const copy={
"zh-CN":{title:"厨房队列",waiting:"待接单",preparing:"备餐中",ready:"待取餐",courier:"配送交接",late:"已超时",priority:"优先级",normal:"普通",high:"优先",urgent:"紧急",eta:"调整时间",minutes:"分钟",empty:"厨房队列为空",overdue:"超时",elapsed:"已用时",refresh:"刷新",customerShipDirect:"客户自付运费"},
"zh-TW":{title:"廚房隊列",waiting:"待接單",preparing:"備餐中",ready:"待取餐",courier:"配送交接",late:"已逾時",priority:"優先級",normal:"普通",high:"優先",urgent:"緊急",eta:"調整時間",minutes:"分鐘",empty:"廚房隊列為空",overdue:"逾時",elapsed:"已用時",refresh:"重新整理",customerShipDirect:"客戶自付運費"},
"vi-VN":{title:"Hàng đợi bếp",waiting:"Chờ nhận",preparing:"Đang chuẩn bị",ready:"Chờ lấy món",courier:"Bàn giao",late:"Đơn trễ",priority:"Ưu tiên",normal:"Bình thường",high:"Ưu tiên",urgent:"Khẩn",eta:"Điều chỉnh thời gian",minutes:"phút",empty:"Không có đơn trong hàng đợi bếp",overdue:"Trễ",elapsed:"Đã xử lý",refresh:"Làm mới",customerShipDirect:"Khách tự trả ship"},
"en-US":{title:"Kitchen queue",waiting:"Waiting",preparing:"Preparing",ready:"Ready",courier:"Handoff",late:"Late",priority:"Priority",normal:"Normal",high:"High",urgent:"Urgent",eta:"Adjust time",minutes:"minutes",empty:"Kitchen queue is empty",overdue:"Late",elapsed:"Elapsed",refresh:"Refresh",customerShipDirect:"Customer pays courier"}} as const;
...
export default function KitchenQueue({organizationId}:{organizationId:string}){
 ...
 return <section style={{margin:"18px 0"}}>
   ...
   <div className="zx-kitchen-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(220px,1fr))",gap:10,overflowX:"auto",paddingTop:10}}>
    {columns.map(col=><section className="zx-kitchen-column" key={col.key} style={{minWidth:220,alignSelf:"start",background:"#f8faf9",border:"1px solid #e2e8e5",borderRadius:16,padding:9}}>
      ...
      <div style={{display:"grid",gap:8}}>{!col.items.length?<small style={{padding:"10px 4px",color:"#94a3b8",textAlign:"center"}}>{t.empty}</small>:col.items.map(item=><article key={item.requestId} style={{padding:10,borderRadius:13,background:item.late?"#fff1f2":"#fff",border:item.late?"1px solid #fecdd3":"1px solid #edf2ef",boxShadow:"0 5px 14px rgba(15,23,42,.04)"}}>
        ...
        {Boolean(item.deliveryProvider)&&<div style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 8px",borderRadius:999,background:item.deliveryProvider==="grab"?"#dcfce7":"#ccfbf1",color:item.deliveryProvider==="grab"?"#15803d":"#0f766e",fontSize:11,fontWeight:850,marginTop:4}}><img src={item.deliveryProvider==="grab"?"/couriers/grab.png":"/couriers/green-sm.png"} alt={item.deliveryProvider==="grab"?"Grab":"Green SM"} style={{height:13,width:"auto",objectFit:"contain"}}/><span>{item.deliveryProvider==="grab"?"Grab":"Xanh SM"} ({t.customerShipDirect})</span></div>}
        <small style={{display:"block",color:"#64748b",marginTop:4}}>⏱ {t.elapsed}: {item.elapsedMinutes} {t.minutes}</small>
        <div style={{display:"flex",gap:5,marginTop:8}}>{(["normal","high","urgent"] as const).map(level=><button key={level} disabled={!!busy} onClick={()=>void patch(item,{action:"priority",priority:level})} style={{flex:1,border:0,borderRadius:8,padding:"5px 3px",whiteSpace:"nowrap",background:item.priority===level?(level==="urgent"?"#fee2e2":"#dcfce7"):"#f1f5f9",color:item.priority===level?(level==="urgent"?"#b91c1c":"#067647"):"#64748b",fontSize:13,fontWeight:850}}>{t[level]}</button>)}</div>
