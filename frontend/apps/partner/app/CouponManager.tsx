...
export default function CouponManager({organizationId}:{organizationId:string}){
 ...
 async function load(){if(!organizationId)return;const r=await fetch(`/api/partner-coupons?organizationId=${encodeURIComponent(organizationId)}`,{cache:"no-store"});const j=await r.json();if(j?.ok)setRows(j.data||[])}
 useEffect(()=>{void load()},[organizationId]);
 async function create(){setMsg("");const r=await fetch("/api/partner-coupons",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...form,organizationId,maxDiscountAmount:form.maxDiscountAmount===""?null:Number(form.maxDiscountAmount),totalUsageLimit:form.totalUsageLimit===""?null:Number(form.totalUsageLimit),startsAt:form.startsAt?new Date(form.startsAt).toISOString():null,endsAt:form.endsAt?new Date(form.endsAt).toISOString():null})});const j=await r.json();if(r.ok&&j?.ok){setForm(blank);await load()}else setMsg(j?.error?.code||"ERROR")}
