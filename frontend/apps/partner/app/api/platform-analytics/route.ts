...
export async function GET(request:NextRequest){
  try {
    ...
    return EdgeNextResponse.json(payload || { ok: false, error: { code: "ANALYTICS_UNAVAILABLE" } }, { status: response.status });
  } catch {
    return EdgeNextResponse.json({ ok: false, error: { code: "ANALYTICS_UNAVAILABLE" } }, { status: 503 });
  }
}
