...
export async function PATCH(request:NextRequest,context:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    return EdgeNextResponse.json(result||{ok:false,error:{code:"UPDATE_STATUS_FAILED"}},{status:response.status});
  }catch{return EdgeNextResponse.json({ok:false,error:{code:"UPDATE_STATUS_UNAVAILABLE",message:"Backend unavailable"}},{status:503})}
}
