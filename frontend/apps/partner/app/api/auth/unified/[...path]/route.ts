...
async function handle(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (
    r.status === 401 &&
    ((method === "GET" && (path === "session/me" || path === "session/devices")) ||
      path.startsWith("identity/otp/"))
  ) {
    const refreshed = await refreshWithCookie(request);
    if (refreshed) {
      access = String(refreshed.accessToken || "");
      try {
        r = await upstream(path, method, method === "POST" ? body : undefined, access);
        payload = await responsePayload(r);
      } catch (err) {
        return NextResponse.json(
          {
      ...
      if (path === "identity/otp/verify" && payload?.ok) {
        const verified = NextResponse.json(
          { ok: true, data: payload.data.session },
          { status: r.status, headers: { "cache-control": "no-store" } }
        );
        setAuthCookies(verified, payload.data);
        if (payload.data?.trustedDeviceToken) {
          verified.cookies.set(
            TRUSTED_COOKIE,
  ...
  if (
    ...
    path === "identity/otp/verify" ||
    path === "identity/pin/login" ||
      ...
      payload?.ok
        ? {
            ...
            data: {
              ...
              needsProfileCompletion: payload.data.needsProfileCompletion,
