...
function Content(){
 ...
 const load=useCallback(async()=>{if(!id)return;try{const[orderResponse,paymentResponse]=await Promise.all([fetch(`/api/platform-requests/${encodeURIComponent(id)}?locale=${locale}`,{cache:"no-store"}),fetch(`/api/platform-payments?requestId=${encodeURIComponent(id)}`,{cache:"no-store"})]);const order=await orderResponse.json();const payments=await paymentResponse.json();if(order?.data?.status)setStatus(order.data.status);const row=Array.isArray(payments?.data)?payments.data[0]:null;if(row)setPayment(row)}catch{}},[id,locale]);
 ...
 return <main className={styles.shell}><section className={styles.success}><div>✓</div><small>ZHAOXI</small><h1>{t.title}</h1><p>{t.success}{partner?` — ${partner}`:""}</p><label>{t.code}</label><strong>{code}</strong><span>{statusLabels[locale][status]||status}</span>
 {payment&&<div style={{width:"100%",marginTop:14,padding:16,border:"1px solid #dce7e0",borderRadius:18,background:"#fff",textAlign:"center"}}><b>💳 {paymentMethodLabel(payment.method,locale)}</b><p style={{margin:"6px 0",color:payment.status==="paid"?"#07a856":"#64748b"}}>{paymentStatusLabel(payment.status,locale)}</p>{payment.method==="wechat_pay"&&payment.status!=="paid"&&<>{checkout.qrDataUrl?<><img src={checkout.qrDataUrl} alt={t.payTitle} style={{width:220,height:220,maxWidth:"100%",borderRadius:14}}/><p style={{margin:"8px 0 2px"}}>{t.scan}</p><small>{t.expires}</small></>:creating?<p>{t.preparing}</p>:payError?<p style={{color:"#b42318"}}>{t.payError}</p>:null}</>}{payment.status==="paid"&&<strong style={{display:"block",color:"#07a856",marginTop:8}}>✓ {t.paid}</strong>}</div>}
