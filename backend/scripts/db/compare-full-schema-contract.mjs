...
const prodPath = path.resolve('artifacts/schema/production-schema-full.json');
...
const sameIndexShape = (a,b) => Boolean(a.unique) === Boolean(b.unique) && (a.columns||[]).map(clean).join(',') === (b.columns||[]).map(clean).join(',');
const parseFk = def => {
...
const result={generatedAt:new Date().toISOString(),scope:'FULL_CONTRACT_B1_2_SEMANTIC',counts,byKind,informationalByKind,missingTables,extraTables,issues,informational};
const out=path.resolve('artifacts/schema/schema-full-diff.json');
fs.writeFileSync(out,JSON.stringify(result,null,2)+'\n');
console.log(`Full schema contract diff written: ${out}`);
